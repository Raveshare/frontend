...
import { refreshNFT, solanaAuth, evmAuth } from '../services/apis/BE-apis/backendApi'
...
import { BraveShieldWarn, CheckInternetConnection, LoadingComponent, OnboardingSteps, OnboardingStepsWithShare, UpdateAvailable } from './editor/common'
...
const App = () => {
	...
	const { isLoading, setIsLoading, text, setText, posthog, dialogOpen, explorerLink, handleOpen, setOpenedLoginModal } = useContext(Context)
	const [sign, setSign] = useState('')
	const { address, isConnected, isDisconnected } = useAccount()
	const { disconnect } = useDisconnect()
	const { data: evmSignature, isError, isSuccess, error, signMessage } = useSignMessage()
	...
	const { solanaConnected, solanaSignMessage, solanaAddress, solanaDisconnect } = useSolanaWallet()
	const [solanaSignature, setSolanaSignature] = useState('')
	const { solanaWalletError, setSolanaWalletError } = useContext(SolanaWalletErrorContext)
	...
	const isUserEligible = () => {
		if (getifUserEligible && getifUserEligible.address === address && getifUserEligible.isUserEligible === true) {
			return true
